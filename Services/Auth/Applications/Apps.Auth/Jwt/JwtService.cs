...
public interface IJwtService {
    ...
    Task<AccountResult> EvaluateAsync(string token , string userId);
}
...
internal class JwtService(JwtSettingsModel model) : IJwtService {
    ...
    public async Task<AccountResult> EvaluateAsync(string token , string userId) {
        ...
        if(userId != userIdValue) {
           throw JwtException.Create("Your jwt token is invalid.");
        }
